...
files = dir(fullfile(pwd,wingRudder,'*.csv'));
if ispc
    [~,~,data]=xlsread(fullfile(pwd,wingRudder,files(1).name));
    aeroTable.fileName = files(1).name;
...
% This is what I get from weissiger method induced drag calculation
% aeroTable.cd    = data(:,3)+((aeroTable.cl).^2)./(pi*p.AR);
